...
def _analyze_image_object(img) -> dict:
    client = _get_client()
...
def analyze_dress_image(image_path: str) -> dict:
    """
    Backward-compatible file-path analyzer used by existing routes.
    """
    try:
        with Image.open(image_path) as img:
            return _analyze_image_object(img)
    except Exception as e:
        logger.error(f"Error in analyze_dress_image: {e}")
        return _fallback_analysis()

def analyze_dress_image_url(image_url: str) -> dict:
    """
    Sends the dress image from a URL to Gemini Vision API and returns a structured JSON metadata.
    """
    try:
        # Load image from URL
        ...
        logger.error(f"Error in analyze_dress_image_url: {e}")
        return _fallback_analysis()
